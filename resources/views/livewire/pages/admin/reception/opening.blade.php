            ...
            <x-card class="mt-2" >
                <x-slot name="header">
                    ...
                    <h5 class="text-xl font-medium text-gray-900 dark:text-white">Pembukaan Program Pembelajaran</h5>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Silahkan masukan data pembukaan program pembelajaran.</p>
                    </div>
                </x-slot>
            ...
            <x-card class="mt-2 w-full ">
                <x-slot name="header">
                    ...
                    <h5 class="text-xl font-medium text-gray-900 dark:text-white">Daftar Program Pembelajaran Periode {{ $this->reception->period }}</h5>
                        <p class="text-sm text-gray-600 dark:text-gray-300">Daftar yang dibuka pada periode {{ $this->reception->period }}</p>
                    </div>
                </x-slot>
                            ...
                            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 space-y-2">
                                    <div class="flex flex-nowrap gap-1">
                                        <x-button class="inline-flex" color="yellow-outline" wire:click="edit({{ $opening->id }})">
                                            <x-icons.edit class="w-4 mr-1" /> Edit
                                        </x-button>
                                        <x-button class="inline-flex" color="red-outline" wire:click="destroy({{ $opening->id }})" wire:confirm="Data akan dihapus, Yakin?">
