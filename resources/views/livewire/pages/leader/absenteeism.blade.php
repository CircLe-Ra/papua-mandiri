...
usesPagination();
layout('layouts.app');
state(['period' => '','level' => 1])->url(keep: false);
state(['time_id','program_id']);
            ...
            <x-card class="mt-2 w-full ">
                ...
                <x-slot name="sideHeader" >
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-2 col-span-4">
                        ...
                        <x-form.input-select id="program_id" name="program_id" wire:model="program_id" size="xs" class="w-full" get-data="server" :data="$this->programs" display_name_first="Program?" :alert="false" />
                        ...
                        <x-button wire:click="print" color="blue" size="xs" class="lg:col-span-1" :disabled="count($this->participants) <= 0"
